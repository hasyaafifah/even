...
class Tabel_Event extends RestController
{

    public function index_get($id = 0)
    {
        $check_data = $this->db->get_where('tb_event', ['id' => $id])->row_array();

        if ($id) {
            if ($check_data) {
                $data = $this->db->get_where('tb_event', ['id' => $id])->row_array();
            ...
            $data = $this->db->get('tb_event')->result();
